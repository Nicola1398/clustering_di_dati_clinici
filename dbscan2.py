...
print(size_features*2) #per minPts

#DBSCAN
neigh = NearestNeighbors(n_neighbors=size_features*2)
...
plt.ylabel("eps")
plt.plot(distances)
plt.savefig("immagini/DBSCAN2/eps.pdf")
#plt.show()
plt.close()
...
plt.ylabel("eps")
plt.savefig("immagini/DBSCAN2/kneepoint.pdf")
#plt.show()
plt.close()
...
min_samples_to_test = range(40, 90, 10)
...
def get_metrics(eps, min_samples, dataset, iter_):
    ...
    if True in noise_indices:
        neighboors = NearestNeighbors(n_neighbors=size_features*2).fit(dataset)
...
plt.tight_layout();
plt.savefig("immagini/DBSCAN2/heatmapDBSCAN.pdf")
#plt.show()
plt.close()

# define the model
dbscan_model = DBSCAN(eps=distances[knee.knee], min_samples=size_features*2)
...
for i in range (0,size_features,1):
    ...
    plt.savefig("immagini/DBSCAN2/%s.pdf" %(features[i]))
    #plt.show()
    plt.close()
