...
plt.savefig("immagini/Agglomerative2_pca_target/2pca.pdf")
plt.close()
...
plt.savefig("immagini/Agglomerative2_pca_target/Dendrogramma.pdf")
#plt.show()
plt.close()

model = AgglomerativeClustering(n_clusters=2)
model.fit(principalDf) #senza target
label = model.labels_
print(label)
...
for i in range (0,size_finalDF-1,1):
    ...
    plt.savefig("immagini/Agglomerative2_pca_target/%s.pdf" %(finalDf.columns[i]))
    #plt.show()'''
    plt.close()
...
#plt.xlabel("Cluster")
plt.xticks(np.arange(2), ('cluster 0\nN.patients: %d\nMedian: %.2f' %(shape0[0], median0), 'cluster 1\nN.patients: %d\nMedian: %.2f' %(shape1[0], median1)))
plt.savefig("immagini/Agglomerative2_pca_target/Time of life.pdf")
#plt.show()
plt.close()